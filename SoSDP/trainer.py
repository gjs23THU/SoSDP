...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    valid_loader = DataLoader(valid_data, batch_size, True, num_workers=0)
    ...
    for epoch in range(5):
        ...
        for batch_idx, batch in enumerate(train_iter):
            ...
            dynamic = dynamic.to(device)
            x0 = x0.to(device) if len(x0) > 0 else None
            # print("batch_idx为：",batch_idx,"static为：",static[-1,:,-1])
            ...
            if (batch_idx + 1) % 100 == 0:
                ...
                print('  Batch %d/%d, reward: %2.3f, loss: %2.4f, took: %2.4fs' %
                      (batch_idx, len(train_loader), mean_reward, mean_loss,
                       times[-1]))
                epoch_dir1 = os.path.join(epoch_dir, '%s' % batch_idx)
                if not os.path.exists(epoch_dir1):
                    os.makedirs(epoch_dir1)
                save_path = os.path.join(epoch_dir1, 'actor.pt')
                torch.save(actor.state_dict(), save_path)

                save_path = os.path.join(epoch_dir1, 'critic.pt')
...
def train_SoS(args):
    ...
    from tasks import SoS
    from tasks.SoS import SoSDataset

    STATIC_SIZE = 13 # (x, y)
    DYNAMIC_SIZE = 1 # dummy for compatibility

    train_data = SoSDataset(args.num_nodes,args.capnum, args.train_size, args.seed)
    valid_data = SoSDataset(args.num_nodes,args.capnum, args.valid_size, args.seed + 1)

    update_fn = None

    actor = DRL4SoS(STATIC_SIZE,
                    DYNAMIC_SIZE,
                    args.hidden_size,
                    update_fn,
                    SoS.update_mask,
    ...
    test_data = SoSDataset(args.num_nodes,args.capnum, args.train_size, args.seed + 2)

    test_dir = 'test'
    test_loader = DataLoader(test_data, args.batch_size, False, num_workers=0)
    out = validate(test_loader, actor, SoS.reward, SoS.render, test_dir, num_plot=5)

    print('Average cost: ', out)
...
def test(args):
    ...
    train_data = SoSDataset(args.num_nodes,args.capnum, 10, args.seed+2)
    update_fn = None
    start=time.time()
    Actor1 = DRL4SoS(STATIC_SIZE,
                    DYNAMIC_SIZE,
                    args.hidden_size,
                    update_fn,
                    SoS.update_mask,
    ...
    Actor1.load_state_dict(torch.load(PATH1))
    Actor1.eval()
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader, desc='Test', leave=False)):
            ...
            # Full forward pass through the dataset
            tour_indices, tour_logp = Actor1(static, dynamic, x0)
            reward=SoS.reward(static,tour_indices)
            rewards.append(reward)


            A=static.squeeze(0).cpu().numpy().T
            data = pd.DataFrame(A)
            # writer = pd.ExcelWriter('A.xlsx')		
            # print(batch_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', default='SoS')
